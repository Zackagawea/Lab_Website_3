...
app.get('/player_info/select_player', function(req, res) {
        var input = req.query.player_choice;
	var query1 = 'SELECT id,name FROM football_players;';
	var query2 = 'SELECT * FROM football_players WHERE id = ' + input + ';';
	var query3 = 'SELECT COUNT(*) FROM football_players INNER JOIN football_games ON football_players.id = ANY(football_games.players) WHERE id=' + input + ';';
	db.task('get-everything', task => {
	    return task.batch([
		task.any(query1),
		task.any(query2),
